...
**原稿の書き方（声・改行・見出しのルール）は [BLOG_WRITING_GUIDE.md](BLOG_WRITING_GUIDE.md)。** ちびあっきーが原稿を作る時もそれが正本。
...
見出しには自動でidが付く（ページ内リンク可）。H1（`#`）は使わない（タイトルがH1）。大見出し `##`、その中の項目 `###`。
**1行改行はそのまま改行になる**（短い行で息をつく書き方がそのまま出る）。段落を分けたい時は空行。
